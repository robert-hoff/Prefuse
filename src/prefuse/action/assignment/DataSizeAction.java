...
public class DataSizeAction extends SizeAction {
  ...
  /**
   * Set the scale (linear, square root, or log) to use for encoding size values
   * from the data.
   * 
   * @param scale
   ...
   *          {@link prefuse.Constants#QUANTILE_SCALE} is used, the number of bins
   *          to use must also be specified to a value greater than zero using the
   ...
   * 
   * @param count
   *          the number of bins to set. The value {@link Constants#CONTINUOUS}
   *          indicates not to use any binning. If the scale type set using the
   *          {@link #setScale(int)} method is {@link Constants#QUANTILE_SCALE},
   *          the bin count <strong>must</strong> be greater than zero.
   */
  public void setBinCount(int count) {
    if (m_scale == Constants.QUANTILE_SCALE && count <= 0) {
      throw new IllegalArgumentException(
          "The quantile scale can not be used without binning. " + "Use a bin value greater than zero.");
    }
    m_bins = count;
  }

  /**
   * Indicates if the size values set by this function represent 2D areas. That
   * is, if the size is a 2D area or a 1D length. The size value will be scaled
   * appropriately to facilitate better perception of size differences.
   * 
   * @return true if this instance is configured for area sizes, false for length
   *         sizes.
  ...
  /**
   * Sets if the size values set by this function represent 2D areas. That is, if
   * the size is a 2D area or a 1D length. The size value will be scaled
   * appropriately to facilitate better perception of size differences.
   * 
   * @param isArea
   *          true to configure this instance for area sizes, false for length
   *          sizes
   * @see prefuse.util.PrefuseLib#getSize2D(double)
   */
  ...
  /**
   * Gets the maximum size value that will be assigned by this action. By default,
   * the maximum size value is determined automatically from the data, faithfully
   * representing the scale differences between data values. However, this can
   * sometimes result in very large differences. For example, if the minimum data
   * value is 1.0 and the largest is 200.0, the largest items will be 200 times
   * larger than the smallest. While accurate, this may not result in the most
   * readable display. To correct these cases, use the
   * {@link #setMaximumSize(double)} method to manually set the range of allowed
   * sizes.
   * 
   ...
   *         have already been run (allowing the value to be automatically
   *         computed) or the maximum size must have been explicitly set.
   */
  ...
  /**
   * Set the maximum size value that will be assigned by this action. By default,
   * the maximum size value is determined automatically from the data, faithfully
   * representing the scale differences between data values. However, this can
   * sometimes result in very large differences. For example, if the minimum data
   * value is 1.0 and the largest is 200.0, the largest items will be 200 times
   ...
   * 
   * @param maxSize
   *          the maximum size to use. If this value is less than or equal to
   *          zero, infinite, or not a number (NaN) then the input value will be
   *          ignored and instead automatic inference of the size range will be
   *          performed.
   */
    ...
    if (m_inferBounds) {
      if (m_scale == Constants.QUANTILE_SCALE && m_bins > 0) {
        ...
        if (m_scale == Constants.QUANTILE_SCALE) {
          Logger.getLogger(getClass().getName()).warning("Can't use quantile scale with no binning. "
              + "Defaulting to linear scale. Set the bin value " + "greater than zero to use a quantile scale.");
