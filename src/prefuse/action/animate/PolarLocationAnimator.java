...
public class PolarLocationAnimator extends ItemAction {
  ...
  /**
   * Creates a PolarLocationAnimator that operates on VisualItems within the
   * specified group, while using regular linear interpolation (in Cartesian (x,y)
   * coordinates rather than polar coordinates) for items also contained within
   * the specified linearGroup.
   * 
   * @param group
   *          the data group to process
   * @param linearGroup
   *          the group of items that should be interpolated in Cartesian
   *          (standard x,y) coordinates rather than polar coordinates. Note that
   *          this animator will not process any items in <code>linearGroup</code>
   *          that are not also in <code>group</code>.
   */
