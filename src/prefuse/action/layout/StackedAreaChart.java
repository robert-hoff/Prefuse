...
public class StackedAreaChart extends Layout {
   ...
   * @param columns
   *          the various data fields, in sorted order, that should be referenced
   *          for each consecutive point of a stack layer
   */
   ...
   *          for each consecutive point of a stack layer
   * @param threshold
   *          height threshold under which stacks should not be made visible.
   */
  ...
  /**
   * Gets the percentage of the layout bounds that should be reserved for empty
  ...
  /**
   * Sets the percentage of the layout bounds that should be reserved for empty
   * space at the top of the stack.
   * 
   * @param p
  ...
  private float[] getPolygon(VisualItem item, String field) {
    ...
    if (poly == null || poly.length < 4 * columns.length) {
      ...
      float inc = (float) (m_horiz ? (bounds.getMinY() - bounds.getMaxY()) : (bounds.getMaxX() - bounds.getMinX()));
      inc /= len - 1;
      float max = (float) (m_horiz ? (m_top ? bounds.getMaxX() : bounds.getMinX())
          : (m_top ? bounds.getMinY() : bounds.getMaxY()));
      float min = (float) (m_horiz ? bounds.getMaxY() : bounds.getMinX());
