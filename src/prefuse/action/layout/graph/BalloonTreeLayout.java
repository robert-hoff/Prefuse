...
public class BalloonTreeLayout extends TreeLayout {
   ...
   * 
   * @param group
   *          the data group to layout. Must resolve to a Graph or Tree instance.
   */
   ...
   * np.f/numChildren); double pr = 0; double bx = 0, by = 0; Iterator childIter =
   ...
   * numChildren != 0 ) { bx /= numChildren; by /= numChildren; } np.rx = -bx;
   * np.ry = -by;
   * 
   * p = Math.PI; pr = 0; np.r = 0; childIter = n.getChildren(); while (
   * childIter.hasNext() ) { NodeItem c = (NodeItem)childIter.next(); ParamBlock
   * cp = getParams(c); p += pr + cp.a + fs; double x = cp.r*Math.cos(p)-bx;
   * double y = cp.r*Math.sin(p)-by; double d = Math.sqrt(x*x+y*y) + cp.r; np.r =
   * Math.max(np.r, (int)Math.round(d)); pr = cp.a; } if ( np.r == 0 ) np.r =
   * m_minRadius + 2*np.d; } //
   * 
   * private void secondWalk2(NodeItem n, NodeItem r, double x, double y, double
   * l, double t) { ParamBlock np = getParams(n); double cost = Math.cos(t);
   * double sint = Math.sin(t); double nx = x + l*(np.rx*cost-np.ry*sint); double
   * ny = y + l*(np.rx*sint+np.ry*cost); setLocation(n,r,nx,ny); double dd =
   * l*np.d; double p = Math.PI; double fs = np.f / (n.getChildCount()+1); double
   * pr = 0; Iterator childIter = n.getChildren(); while ( childIter.hasNext() ) {
   * NodeItem c = (NodeItem)childIter.next(); ParamBlock cp = getParams(c); double
   * aa = np.c * cp.a; double rr = np.d * Math.tan(aa)/(1-Math.tan(aa)); p += pr +
