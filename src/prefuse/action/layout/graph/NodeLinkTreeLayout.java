...
public class NodeLinkTreeLayout extends TreeLayout {
  ...
  /**
   * Set the offset value for placing the root node of the tree. The dimension in
   * which this offset is applied is dependent upon the orientation of the tree.
   * For example, in a left-to-right orientation, the offset will a horizontal
   * offset from the left edge of the layout bounds.
   * 
   * @param o
  ...
  private double spacing(NodeItem l, NodeItem r, boolean siblings) {
    ...
    return (siblings ? m_bspace : m_tspace) + 0.5 * (w ? l.getBounds().getWidth() + r.getBounds().getWidth()
