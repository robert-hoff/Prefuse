...
public class ForceDirectedLayout extends Layout {
  ...
  /**
   * Create a new ForceDirectedLayout. By default, this layout will not restrict
   * the layout to the layout bounds and will assume it is being run in animated
   * (rather than run-once) fashion.
   * 
   * @param graph
  ...
  /**
   * Create a new ForceDirectedLayout. The layout will assume it is being run in
   * animated (rather than run-once) fashion.
   * 
   * @param group
   *          the data group to layout. Must resolve to a Graph instance.
   * @param enforceBounds
   *          indicates whether or not the layout should require that all node
   *          placements stay within the layout bounds.
   */
   ...
   *          fashion. In run-once mode, the layout will run for a set number of
   *          iterations when invoked. In animation mode, only one iteration of
  ...
  /**
   * Get the maximum timestep allowed for integrating node settings between runs
   * of this layout. When computation times are longer than desired, and node
   * positions are changing dramatically between animated frames, the max step
  ...
  /**
   * Set the maximum timestep allowed for integrating node settings between runs
   * of this layout. When computation times are longer than desired, and node
   * positions are changing dramatically between animated frames, the max step
   * time can be lowered to suppress node movement.
   * 
   * @param maxstep
   *          the maximum timestep allowed for integrating between two layout
   *          steps
   */
  ...
  /**
   * Explicitly sets the node and edge groups to use for this layout, overriding
   * the group setting passed to the constructor.
   * 
   * @param nodeGroup
   *          the node data group
   * @param edgeGroup
  ...
  /**
   * Get the mass value associated with the given node. Subclasses should override
   * this method to perform custom mass assignment.
   * 
   * @param n
   *          the node for which to compute the mass value
   * @return the mass value for the node. By default, all items are given a mass
   *         value of 1.0.
   */
   ...
   * @return the spring length for the edge. A return value of -1 means to ignore
   *         this method and use the global default.
   */
  ...
  /**
   * Get the spring coefficient for the given edge, which controls the tension or
   * strength of the spring. Subclasses should override this method to perform
   * custom spring tension assignment.
   * 
   * @param e
   *          the edge for which to compute the spring coefficient.
   * @return the spring coefficient for the edge. A return value of -1 means to
  ...
  /**
   * Get the referrer item to use to set x or y coordinates that are initialized
   * to NaN.
   * 
  ...
  /**
   * Set the referrer item to use to set x or y coordinates that are initialized
   * to NaN.
   * 
   * @param referrer
