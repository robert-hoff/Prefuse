...
public abstract class Layout extends GroupAction {
  ...
  /**
   * Returns the bounds in which the layout should be computed. If the bounds have
   * been explicitly set, that value is used. Otherwise, an attempt is made to
   * compute the bounds based upon the display region of the first display found
   * in this action's associated Visualization.
   * 
  ...
  /**
   * Explicitly set the layout bounds. A reference to the input rectangle instance
   * is maintained, not a copy, and so any subsequent changes to the rectangle
   * object will also change the layout bounds.
   * 
   * @param b
   *          a rectangle specifying the layout bounds. A reference to this same
   *          instance is kept.
   */
   ...
   * point is used (if it is used at all) is dependent on the particular Layout
   * implementation. If no anchor point has been explicitly set, the center
   * coordinate for the first display found in this action's associated
   ...
   * x-coordinate will be set to the current value, and the current and end values
   * will be set to the provided x-coordinate. If the current value is not a
   * number (NaN), the x-coordinate of the provided referrer item (if non null)
   * will be used to set the start coordinate.
   * 
   * @param item
   *          the item to set
   * @param referrer
   *          the referrer item to use for the start location if the current value
   *          is not a number (NaN)
   * @param x
   *          the x-coordinate value to set. This will be set for both the current
   *          and end values.
   ...
   * y-coordinate will be set to the current value, and the current and end values
   * will be set to the provided y-coordinate. If the current value is not a
   * number (NaN), the y-coordinate of the provided referrer item (if non null)
   * will be used to set the start coordinate.
   * 
   * @param item
   *          the item to set
   ...
   *          is not a number (NaN)
   * @param y
   *          the y-coordinate value to set. This will be set for both the current
   *          and end values.
