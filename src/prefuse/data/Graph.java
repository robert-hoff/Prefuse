...
public class Graph extends CompositeTupleSet {
  ...
  /**
   * Create a new Graph using the provided table of node data and an empty set of
   * edges.
   * 
   * @param nodes
   *          the backing table to use for node data. Node instances of this graph
   *          will get their data from this table.
   * @param directed
   *          true for directed edges, false for undirected
   */
   ...
   *          true for directed edges, false for undirected
   * @param nodeKey
   *          data field used to uniquely identify a node. If this field is null,
   *          the node table row numbers will be used
   * @param sourceKey
  ...
  /**
   * Create a new Graph, using node table row numbers to uniquely identify nodes
   * in the edge table's source and target fields.
   * 
   * @param nodes
   *          the backing table to use for node data. Node instances of this graph
   *          will get their data from this table.
   * @param edges
   *          the backing table to use for edge data. Edge instances of this graph
   *          will get their data from this table.
   * @param directed
   *          true for directed edges, false for undirected
   */
  ...
  public void setTupleManagers(TupleManager ntm, TupleManager etm) {
    if (!Node.class.isAssignableFrom(ntm.getTupleType())) {
      throw new IllegalArgumentException(
          "The provided node " + "TupleManager must generate tuples that implement " + "the Node interface.");
    }
    if (!Edge.class.isAssignableFrom(etm.getTupleType())) {
      throw new IllegalArgumentException(
          "The provided edge " + "TupleManager must generate tuples that implement " + "the Edge interface.");
    }
    m_nodeTuples = ntm;
    m_edgeTuples = etm;
  }
  ...
  /**
   * Dispose of this graph. Unregisters this graph as a listener to its included
   * tables.
   */
  public void dispose() {
  ...
  /**
   * Updates this graph to use a different edge structure for the same nodes. All
   * other settings will remain the same (e.g., directionality, keys)
   * 
   * @param edges
   *          the new edge table.
   */
   ...
   * @param incr
   *          the increment value, 1 for an added link, -1 for a removed link
   */
  ...
  /**
   * Given a node id (a row number in the node table), get the value of the node
   * key field.
   * 
   * @param node
   *          the node id
  ...
  /**
   * Given a value of the node key field, get the node id (the row number in the
   * node table).
   * 
   * @param key
   ...
   * @return true if the node was successfully removed, false if the node id was
   ...
   * @return true if the node was successfully removed, false if the node was not
   *         found in this graph
   */
   ...
   * @return true if the edge was successfully removed, false if the edge was not
   ...
   *          true if this method should throw an Exception when an invalid node
   *          is encountered
  ...
  /**
   * Get the Node tuple corresponding to the input node key field value. The node
   * key field is used to find the node id (node table row number), which is then
   * used to retrieve the Node tuple.
   * 
   * @param key
   *          a node key field value
  ...
  /**
   * Get the in-degree of a node, the number of edges for which the node is the
   * target.
   * 
   * @param node
  ...
  /**
   * Get the out-degree of a node, the number of edges for which the node is the
   * source.
   * 
   * @param node
   *          the node id (node table row number)
  ...
  /**
   * Get the degree of a node, the number of edges for which a node is either the
  ...
  /**
   * Returns an edge from the source node to the target node. This method returns
   * the first such edge found; in the case of multiple edges there may be more.
   */
   ...
   * @param target
   *          the target Node
   * @return an Edge with given source and target nodes, or null if no such edge
   *         is found.
   */
  ...
  /**
   * Get the source node id (node table row number) for the given edge id (edge
   * table row number).
   * 
   * @param edge
   *          an edge id (edge table row number)
  ...
  /**
   * Get the target node id (node table row number) for the given edge id (edge
   * table row number).
   * 
   * @param edge
   *          an edge id (edge table row number)
  ...
  /**
   * Given an edge id and an incident node id, return the node id for the other
   * node connected to the edge.
   * 
   * @param edge
   *          an edge id (edge table row number)
   * @param node
   *          a node id (node table row number). This node id must be connected to
   *          the edge
  ...
  /**
   * Given an Edge and an incident Node, return the other Node connected to the
   * edge.
   * 
   * @param e
   *          an Edge instance
   ...
   * @param node
   *          a node id (node table row number)
   * @return an iterator over all edge ids for edges incident on the given node
   */
   ...
   *          a node id (node table row number)
   * @param direction
   *          the directionality of the edges to include. One of {@link #INEDGES}
   *          (for in-linking edges), {@link #OUTEDGES} (for out-linking edges),
   *          or {@link #UNDIRECTED} (for all edges).
   * @return an iterator over all edge ids for edges incident on the given node
   */
  ...
  /**
   * Get an iterator over all edges that have the given node as a target. That is,
   * edges that link into the given target node.
   * 
   * @param node
   *          a node id (node table row number)
  ...
  /**
   * Get an iterator over all edges that have the given node as a source. That is,
   * edges that link out from the given source node.
   * 
   * @param node
   *          a node id (node table row number)
  ...
  /**
   * Get an iterator over all the edges and nodes of this graph. The iterator will
   * return all edges first, then all nodes.
   * 
   ...
   * been constructed, a SpanningTree rooted at the first valid node found in the
   * node table will be generated.
   * 
   * Spanning trees are generated using an unweighted breadth first search over
   * the graph structure.
   * 
  ...
  /**
   * Returns a spanning tree rooted at the specified node. If the current spanning
   * tree is already rooted at the given node, it is simply returned. Otherwise,
   * the tree is reconstructed at the new root and made the current spanning tree
   ...
   * method on a Tree instance will revert the state to the original rooted tree
   * such that a sbusequent call to getSpanningTree() will return the backing Tree
   * itself.
   * 
   * @see #getSpanningTree()
   * @see #getSpanningTree(Node)
   ...
   * 
   * @param t
   *          the backing table where the change occurred (either a node table or
   *          an edge table)
   * @param first
