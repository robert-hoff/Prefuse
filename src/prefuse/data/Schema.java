...
public class Schema implements Cloneable {
  ...
  /**
   * Create a new schema consisting of the given column names, types, and default
   * column values.
   * 
   * @param names
   *          the column names
   ...
   * schemas from a shared base schema. Cloned copies of a locked Schema will not
  ...
  /**
   * Locks the schema, preventing any additional changes. Locked schemas can not
   * be unlocked! Cloned copies of a locked schema will not inherit this locked
   * status.
   * 
   ...
   *           is either name or type are null or the name already exists in this
   *           schema.
   */
  ...
  /**
   * <p>
   * Add a new interpolated column to this data schema. This actually adds three
   * columns to the schema: a column for the current value of the field, and
   * columns for starting and ending values. During animation or operations spread
   * over a time span, the current value can be interpolated between the start and
   * end values.
   * </p>
   * 
   * <p>
   * The name for the current value column is the name parameter provided to the
   * method. The name for the start and end columns will be determined by the
   * return value of {@link PrefuseLib#getStartField(String)} and
  ...
  /**
   * Set default values for the current, start, and end columns of an interpolated
   * column.
   * 
   * @param field
  ...
  public boolean equals(Object o) {
    ...
    for (int i = 0; i < m_size; ++i) {
      if (!(m_names[i].equals(s.getColumnName(i)) && m_types[i].equals(s.getColumnType(i))
  ...
  /**
   * Indicates if values from a given Schema can be safely assigned to data using
   * this Schema. The input Schema must be less than or equal in length to this
   * Schema, and all contained columns in the given Schema must have matching
   * names and types to this Schema. This method does not consider default values
   * settings or the locked status of the Schemas. For example, if the given
   * Schema has different default values than this one, this will have no impact
   * on the assignability of the two.
   * 
   * @param s
   *          the input Schema
