...
public class Tree extends Graph {
  ...
  /**
   * Add a child node to the given parent node. An edge between the two will also
   * be created.
   * 
   * @param parent
  ...
  /**
   * Remove a child edge from the Tree. The child node and its subtree will also
   * be removed from the Tree.
   * 
   * @param edge
   *          the edge id (edge table row number) of the edge to remove
   * @return true if the edge and attached subtree is successfully removed, false
   *         otherwise
   */
  ...
  /**
   * Get the child index (order number of the child) for the given parent node id
   * and child node id.
   * 
   * @param parent
   *          the parent node id (node table row number)
   * @param child
   *          the child node id (node table row number)
   * @return the index of the child, or -1 if the given child node is not actually
   *         a child of the given parent node, or either node is invalud.
   */
   ...
   * @return the node id of the previous sibling, or -1 if there is no previous
   *         sibling.
   */
   ...
   * @param node
   *          a node id (node table row number)
   * @return the node id of the next sibling, or -1 if there is no next sibling.
   */
   ...
   * @param node
   *          a node id (node table row number)
   * @return the depth of the node in tree. The root node is at a depth level of
   *         0, with each child at a greater depth level. -1 is returned if the
   *         input node id is not in the tree.
   */
  ...
  /**
   * Get an iterator over the edge ids for edges connecting child nodes to a given
   * parent
   * 
   * @param node
   *          the parent node id (node table row number)
   * @return an iterator over the edge ids for edges conencting child nodes to a
   *         given parent
   */
   ...
   * @param n
   *          the parent node
   * @return an iterator over the edges connecting child nodes to a given parent
   */
   ...
   * constructed at an alternative root, this method simply returns a pointer to
   * this Tree instance. If a spanning tree rooted at an alternative node has been
   * created, that tree is returned.
   * 
   ...
   * given root is not the same as that of this Tree, a new spanning tree instance
   * will be constructed, made the current spanning tree for this Tree instance,
   * and returned.
   * 
   * To clear out any generated spanning trees use the clearSpanningTree() method
   * of the Graph class. After calling clearSpanningTree(), the getSpanningTree()
   * method (with no arguments) will return a pointer to this Tree instance
   * instead of any generated spanning trees.
   * 
   * @param root
