...
public class TreeMLWriter extends AbstractGraphWriter {
  ...
  static {
    TYPES.put(int.class, prefuse.data.io.TreeMLReader.Tokens.INT);
    TYPES.put(long.class, prefuse.data.io.TreeMLReader.Tokens.LONG);
    TYPES.put(float.class, prefuse.data.io.TreeMLReader.Tokens.FLOAT);
    TYPES.put(double.class, prefuse.data.io.TreeMLReader.Tokens.REAL);
    TYPES.put(boolean.class, prefuse.data.io.TreeMLReader.Tokens.BOOLEAN);
    TYPES.put(String.class, prefuse.data.io.TreeMLReader.Tokens.STRING);
    TYPES.put(Date.class, prefuse.data.io.TreeMLReader.Tokens.DATE);
  }
  ...
  public void writeGraph(Graph graph, OutputStream os) throws DataIOException {
    ...
    for (int i = 0; i < ns.getColumnCount(); ++i) {
      ...
      xml.tag(prefuse.data.io.TreeMLReader.Tokens.DECL, attr, vals, 2);
    }
    xml.end();
    xml.println();
    ...
    attr[0] = prefuse.data.io.TreeMLReader.Tokens.NAME;
    attr[1] = prefuse.data.io.TreeMLReader.Tokens.VALUE;
    ...
    while (n != null) {
      ...
      if (leaf) {
        xml.start(prefuse.data.io.TreeMLReader.Tokens.LEAF);
      } else {
        xml.start(prefuse.data.io.TreeMLReader.Tokens.BRANCH);
      }
      ...
      if (ns.getColumnCount() > 0) {
        for (int i = 0; i < ns.getColumnCount(); ++i) {
          ...
          if (vals[1] != null) {
            xml.tag(prefuse.data.io.TreeMLReader.Tokens.ATTR, attr, vals, 2);
          }
        }
      }
      n = nextNode(n, xml);
  ...
  /**
   * Find the next node in the depth first iteration, closing off open branch tags
   * as needed.
   * 
   * @param x
   *          the current node
  ...
  /**
   * Checks if all Schema types are compatible with the TreeML specification. The
   * TreeML spec only allows the types <code>int</code>, <code>long</code>,
