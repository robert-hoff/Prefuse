...
public class GraphMLWriter extends AbstractGraphWriter {
  ...
  public void writeGraph(Graph graph, OutputStream os) throws DataIOException {
    ...
    printSchema(xml, prefuse.data.io.GraphMLReader.Tokens.EDGE, es, new String[] { graph.getEdgeSourceField(), graph.getEdgeTargetField() });
    xml.println();

    // print graph contents
    Object graphID = graph.getClientProperty(prefuse.data.io.GraphMLReader.Tokens.ID);
    if (graphID == null) {
      xml.start(prefuse.data.io.GraphMLReader.Tokens.GRAPH, prefuse.data.io.GraphMLReader.Tokens.EDGEDEF, graph.isDirected() ? prefuse.data.io.GraphMLReader.Tokens.DIRECTED : prefuse.data.io.GraphMLReader.Tokens.UNDIRECTED);
    } else {
      xml.start(prefuse.data.io.GraphMLReader.Tokens.GRAPH, new String[] { prefuse.data.io.GraphMLReader.Tokens.EDGEDEF, prefuse.data.io.GraphMLReader.Tokens.ID },
          new String[] { graph.isDirected() ? prefuse.data.io.GraphMLReader.Tokens.DIRECTED : prefuse.data.io.GraphMLReader.Tokens.UNDIRECTED, graphID.toString() }, 2);
    }
    ...
    while (nodes.hasNext()) {
      ...
      if (ns.getColumnCount() > 0) {
        ...
        for (int i = 0; i < ns.getColumnCount(); ++i) {
          ...
          xml.contentTag(prefuse.data.io.GraphMLReader.Tokens.DATA, prefuse.data.io.GraphMLReader.Tokens.KEY, field, n.getString(field));
        }
        xml.end();
      } else {
    ...
    while (edges.hasNext()) {
      ...
      if (es.getColumnCount() > 2) {
        ...
        for (int i = 0; i < es.getColumnCount(); ++i) {
          ...
          xml.contentTag(prefuse.data.io.GraphMLReader.Tokens.DATA, prefuse.data.io.GraphMLReader.Tokens.KEY, field, e.getString(field));
        }
        xml.end();
      } else {
  ...
  private void printSchema(XMLWriter xml, String group, Schema s, String[] ignore) {
    String[] attr = new String[] { prefuse.data.io.GraphMLReader.Tokens.ID, prefuse.data.io.GraphMLReader.Tokens.FOR, prefuse.data.io.GraphMLReader.Tokens.ATTRNAME, prefuse.data.io.GraphMLReader.Tokens.ATTRTYPE };
  ...
  /**
   * Checks if all Schema types are compatible with the GraphML specification. The
   * GraphML spec only allows the types <code>int</code>, <code>long</code>,
   * <code>float</code>, <code>double</code>, <code>boolean</code>, and
   * <code>string</code>.
   * 
   * @param s
   *          the Schema to check
