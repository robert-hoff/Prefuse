...
/**
 * Sends queries to a relational database and processes the results, storing the
 * results in prefuse Table instances. This class should not be instantiated
 * directly. To access a database, the {@link ConnectionFactory} class should be
 * used to retrieve an appropriate instance of this class.
 * 
...
public class DatabaseDataSource {
   ...
   * database. This constructor is only package visible and is not intended for
   * use by application level code. Instead, the {@link ConnectionFactory} class
   * should be used to create any number of DatabaseDataSource connections.
   */
   ...
   * 
   * @param query
   *          the text SQL query to execute
   * @return a Table of the query results
   * @throws DataIOException
   *           if an error occurs while executing the query or adding the query
   *           results in a prefuse Table.
   */
   ...
   *          the text SQL query to execute
   * @param keyField
   *          the field to treat as a primary key, ensuring that this field is
   *          indexed in the resulting table instance.
   * @return a Table of the query results
   * @throws DataIOException
   *           if an error occurs while executing the query or adding the query
   *           results in a prefuse Table.
   ...
   * 
   * @param t
   *          the Table to store the results in. If this value is null, a new
   *          table will automatically be created.
   * @param query
   *          the text SQL query to execute
   * @return a Table of the query results
   * @throws DataIOException
   ...
   *          the text SQL query to execute
   * @param keyField
   *          used to determine if the row already exists in the table
   * @return a Table of the query results
   * @throws DataIOException
   *           if an error occurs while executing the query or adding the query
   *           results in a prefuse Table.
   ...
   *          used to determine if the row already exists in the table
   * @param lock
   *          an optional Object to use as a lock when performing data processing.
   *          This lock will be synchronized on whenever the Table is modified.
   * @return a Table of the query results
   * @throws DataIOException
   *           if an error occurs while executing the query or adding the query
   *           results in a prefuse Table.
  ...
  /**
   * Asynchronously executes a query and stores the results in the given table
   * instance. All data processing is done in a separate thread of execution.
   * 
   * @param t
   ...
   *          the query to execute
   * @param keyField
   *          the primary key field, comparisons on this field are performed to
   *          recognize data records already present in the table.
   */
   ...
   * @param keyField
   *          the primary key field, comparisons on this field are performed to
   *          recognize data records already present in the table. A null value
   *          will result in no key checking.
   * @param lock
   *          an optional Object to use as a lock when performing data processing.
   *          This lock will be synchronized on whenever the Table is modified. A
   *          null value will result in no locking.
   * @param listener
   *          an optional listener that will provide notifications before the
   *          query has been issued and after the query has been processed. This
   *          is most useful for post-processing operations.
   */
  ...
  protected Table process(Table t, ResultSet rset, String key, Object lock) throws DataIOException {
    ...
    // clock out
    long time = System.currentTimeMillis() - timein;
    s_logger.info("Internal query processing completed: " + count + " rows, " + (time / 1000) + "." + (time % 1000)
        + " seconds.");
   ...
   * 
   * @param t
   *          the prefuse Table to check for an existing row
   * @param rset
   *          the ResultSet, set to a particular row, which may or may not have a
   *          matching row in the prefuse Table
   * @param keyField
   *          the key field to look up to check for an existing row
