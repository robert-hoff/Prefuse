 ...
 * class supports a map that allows specific data field / column names to use a
 * custom SQLDataHandler implementation, while maintaining a default handler for
 * any data fields without an entry in the map.
 * 
...
public class CompositeSQLDataHandler implements SQLDataHandler {
  ...
  /**
   * Create a new CompositeSQLDataHandler. Uses a {@link DefaultSQLDataHandler} as
   * the default handler.
   */
   ...
   *          the default data handler to use
   */
   ...
   * 
   * @param columnName
   *          the data field / column name
   * @param handler
   *          the data handler to use for the field
   */
  ...
  /**
   * Remove a custom data handler for a given column name. Subsequent to this
   * method, the column will use the default handler.
   * 
   * @param columnName
   *          the data field / column name
   * @return true if a handler was successfully removed, false if no such custom
   *         handler was found.
   */
  ...
  /**
   * @see prefuse.data.io.sql.SQLDataHandler#process(prefuse.data.Table, int,
  ...
  public void process(Table t, int trow, ResultSet rset, int rcol) throws SQLException {
    ...
    if (m_overrides != null && m_overrides.size() > 0) {
      ...
      SQLDataHandler h = (SQLDataHandler) m_overrides.get(name);
      if (h != null) {
        handler = h;
      }
    }
