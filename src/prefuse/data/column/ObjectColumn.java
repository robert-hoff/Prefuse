...
public class ObjectColumn extends AbstractColumn {
   ...
   * @param defaultValue
   *          the default value for the column. If this value is cloneable, it
   *          will be cloned when assigned as defaultValue, otherwise the input
   *          reference will be used for every default value.
   */
  public ObjectColumn(Class type, int nrows, int capacity, Object defaultValue) {
    ...
    try {
      ...
      if (defaultValue != null) {
        Logger.getLogger(getClass().getName()).fine("Default value of type \"" + defaultValue.getClass().getName()
            + "\" is not " + "cloneable. Using Object reference directly.");
      }
