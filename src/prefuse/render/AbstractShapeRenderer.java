...
public abstract class AbstractShapeRenderer implements Renderer {
  ...
  /**
   * Draws the specified shape into the provided Graphics context, using stroke
   * and fill color values from the specified VisualItem. This method can be
   * called by subclasses in custom rendering routines.
   */
  ...
  /**
   * Returns the shape describing the boundary of an item. The shape's coordinates
   * should be in abolute (item-space) coordinates.
   * 
   * @param item
  ...
  /**
   * Retursn the stroke to use for drawing lines and shape outlines. By default
   * returns the value of {@link VisualItem#getStroke()}. Subclasses can override
   * this method to implement custom stroke assignment, though changing the
   * <code>VisualItem</code>'s stroke value is preferred.
   * 
   * @param item
   *          the VisualItem
  ...
  /**
   * Returns a value indicating if a shape is drawn by its outline, by a fill, or
   ...
   * 
   * @param type
   *          the new rendering type. Should be one of {@link #RENDER_TYPE_NONE},
   *          {@link #RENDER_TYPE_DRAW}, {@link #RENDER_TYPE_FILL}, or
