...
public class ImageFactory {
   ...
   * 
   * @param b
   *          true for asynchronous (background) loading, false for synchronous
   *          (blocking) loading
   */
  ...
  /**
   * Sets the maximum image dimensions of loaded images, images larger than these
   * limits will be scaled to fit within bounds.
   * 
   * @param width
   *          the maximum width of input images (-1 means no limit)
   * @param height
   ...
   * already been loaded, it simply will return the image, otherwise it will load
   * it from the specified location.
   * </p>
   * 
   * <p>
   * The imageLocation argument must be a valid resource string pointing to either
   * (a) a valid URL, (b) a file on the classpath, or (c) a file on the local
   * filesystem. The location will be resolved in that order.
   * </p>
   * 
   * @param imageLocation
  ...
  /**
   * Adds an image associated with a location string to this factory's cache. The
   * image will be scaled as dictated by this current factory settings.
   * 
   * @param location
   *          the location string uniquely identifying the image
   * @param image
   *          the actual image
   * @return the final image added to the cache. This may be a scaled version of
  ...
  /**
   * <p>
   * Preloads images for use in a visualization. Images to load are determined by
   * taking objects from the given iterator and retrieving the value of the
