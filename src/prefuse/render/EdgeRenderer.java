...
public class EdgeRenderer extends AbstractShapeRenderer {
   ...
   * 
   * @param edgeType
   *          the edge type, one of {@link prefuse.Constants#EDGE_TYPE_LINE} or
   ...
   *          {@link prefuse.Constants#EDGE_TYPE_CURVE}.
   * @param arrowType
   *          the arrow type, one of {@link prefuse.Constants#EDGE_ARROW_FORWARD},
   ...
   * position and orientation specified by the provided line segment end points.
   */
  ...
  /**
   * Returns the line width to be used for this VisualItem. By default, returns
   * the base width value set using the {@link #setDefaultLineWidth(double)}
   * method, scaled by the item size returned by {@link VisualItem#getSize()}.
   * Subclasses can override this method to perform custom line width
   * determination, however, the preferred method is to change the item size value
   * itself.
   * 
   * @param item
  ...
  /**
   * Returns the stroke value returned by {@link VisualItem#getStroke()}, scaled
   * by the current line width determined by the {@link #getLineWidth(VisualItem)}
   * method. Subclasses may override this method to perform custom stroke
   * assignment, but should respect the line width paremeter stored in the
   * {@link #m_curWidth} member variable, which caches the result of
   ...
   * this method to provide custom curve specifications. To reduce object
   * initialization, the entries of the Point2D array are already initialized, so
   * use the <tt>Point2D.setLocation()</tt> method rather than
   ...
   * @param cp
   *          array of Point2D's (length >= 2) in which to return the control
   *          points
   * @param x1
   ...
   * {@link prefuse.Constants#EDGE_ARROW_FORWARD} for arrows from source to target
   * on directed edges, or {@link prefuse.Constants#EDGE_ARROW_REVERSE} for arrows
   * from target to source on directed edges.
   * 
   * @param type
   *          the new arrow type
   */
   ...
   * pixel dimensions when both the zoom level and the size factor (a combination
   * of item size value and default stroke width) are 1.0.
   * 
   * @param width
   *          the untransformed arrow head width, in pixels. This specifies the
   *          span of the base of the arrow head.
   * @param height
   *          the untransformed arrow head height, in pixels. This specifies the
   *          distance from the point of the arrow to its base.
   */
  ...
  /**
   * Get the width of the untransformed arrow head. This is the length, in pixels,
   * of the base of the arrow head.
   * 
  ...
  /**
   * Sets the default width of lines. This width value will be scaled by the value
   * of an item's size data field. The default base width is 1.
   * 
   * @param w
  ...
  /**
   * Gets the default width of lines. This width value that will be scaled by the
