...
public class FocusControl extends ControlAdapter {
  ...
  /**
   * Creates a new FocusControl that changes the focus to another item when that
   * item is clicked once.
   */
  public FocusControl() {
    this(1);
  }
  ...
  /**
   * Creates a new FocusControl that changes the focus when an item is clicked the
   * specified number of times. A click value of zero indicates that the focus
   * should be changed in response to mouse-over events.
   * 
   * @param clicks
  ...
  /**
   * Set a filter for processing items by this focus control. Only items for which
   * the predicate returns true (or doesn't throw an exception) will be considered
   * by this control. A null value indicates that no filtering should be applied.
   * That is, all items will be considered.
   * 
   * @param p
