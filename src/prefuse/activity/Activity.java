...
public abstract class Activity {
   ...
   * 
   * @param duration
   *          the length of this activity. A value of {@link #INFINITY} indicates
   *          an infinite running time.
   ...
   * Activity will be scheduled to start immediately after the first one finishes,
   * overwriting any previously set startTime. If the first Activity is cancelled,
   * this one will not run.
   * 
   * This functionality is provided by using an ActivityListener to monitor the
   * first Activity. The listener is removed upon completion or cancellation of
   * the first Activity.
   * 
   * This method does not in any way affect the scheduling of the first Activity.
   * If the first Activity is never scheduled, this Activity will correspondingly
   * never be run unless scheduled by a separate scheduling call.
   * 
   * @param before
   ...
   * 
   * This functionality is provided by using an ActivityListener to monitor the
   * first Activity. The listener will persist across mulitple runs, meaning the
   * second Activity will always be evoked upon a successful finish of the first.
   * 
   * This method does not in any way affect the scheduling of the first Activity.
   * If the first Activity is never scheduled, this Activity will correspondingly
   * never be run unless scheduled by a separate scheduling call.
  ...
  /**
   * Run this activity one step. Subclasses should override this method to specify
   * the actions this activity should perform.
   * 
   * @param elapsedTime
   ...
   * ActivityManager -- outside code should have no need to call or override this
   * method. To implement custom activities, override the run() method instead.
   * 
   * @param currentTime
   *          the time at which this step is being run.
   * @return the time (in milliseconds) when this activity should be run again. A
   *         return value of -1 indicates this activity is finished.
   */
  ...
  /**
   * Cancels this activity, if scheduled. This will stop a running activity, and
   * will remove the activity from the ActivityManager's schedule.
   */
  public void cancel() {
    /*
     * Prefuse Bug ID #1708926 The fix ("Contribution") has not been tested and/or
     * validated for release as or in products, combinations with products or other
     * commercial use. Any use of the Contribution is entirely made at the user's
     * own responsibility and the user can not rely on any features, functionalities
     * or performances Alcatel-Lucent has attributed to the Contribution. THE
     * CONTRIBUTION BY ALCATEL-LUCENT (...) IS PROVIDED AS IS, WITHOUT WARRANTY OF
  ...
  /**
   * Sets whether or not this Activity has been scheduled. This method should only
   * be called by the ActivityManager.
   * 
   * @param s
  ...
  /**
   * Returns a value between 0 and 1 inclusive, indicating the current position in
   * an animation or other similarly parameterized activity. The returned value is
   * determined by consulting this Activity's pacing function.
   * 
   * @param elapsedTime
  ...
  /**
   * Returns the pacing function associated with this Activity. Pacing functions
   * are used to control the pace of animations.
   * 
   * @return this Activity's pacing function. A value of null indicates a basic,
   *         linear pace is used, moving from 0 to 1 uniformly over time.
   */
  ...
  /**
   * Sets the pacing function associated with this Activity. Pacing functions are
   * used to control the pace of animations.
   * 
   * @param pfunc
   *          this Activity's new pacing function, or null to indicate a basic,
   *          linear pace moving from 0 to 1 uniformly over time.
   */
   ...
   * 
   * @param duration
   *          The new duration, in milliseconds, for this activity. A value of
   *          {@link #INFINITY} indicates that this activity should run
   *          indefinitely.
   */
