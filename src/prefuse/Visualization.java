 ...
 * visualization, associating it with a provided <em>data group name </em>. This
 * group name allows for queries to this visualization that consider only
 * VisualItem instances from that particular group. This is quite useful when
 * crafting {@link prefuse.action.Action} instances that process only a
 ...
 * 
 * <p>
 * All of the examples discussed above are examples of <em>primary, or visual,
 * data groups</em> of VisualItems. Visualizations also support <em>secondary,
 * or focus data groups</em> that maintain additional collections of the
 * VisualItems stored in the primary groups. Examples include a set of focus
 * items (such as those that have been clicked by the user), selected items
 * (items selected by a user), or search items (all matches to a search query).
 * The exact semantics of these groups and the mechanisms by which they are
 * populated is determined by application creators, but some defaults are
...
public class Visualization {
   ...
   * visualization. An exception will be thrown if the group name is already in
   * use.
   * 
   * @param group
   ...
   *          the data to visualize
   * @param filter
   *          a filter Predicate determining which data Tuples in the input data
   *          set are visualized
   * @return a visual abstraction of the input data, a VisualTupleSet instance
   */
  ...
  /**
   * Add an empty VisualTable to this visualization, using the given data group
   * name. This adds a group of VisualItems that do not have a backing data set,
   * useful for creating interactive visual objects that do not represent data. An
   * exception will be thrown if the group name is already in use.
   * 
   * @param group
   *          the data group name for the visualized data
  ...
  /**
   * Add an empty VisualTable to this visualization, using the given data group
   * name and table schema. This adds a group of VisualItems that do not have a
   * backing data set, useful for creating interactive visual objects that do not
   * represent data. An exception will be thrown if the group name is already in
   * use.
   * 
   * @param group
   *          the data group name for the visualized data
  ...
  /**
   * Adds a data table to this visualization, using the given data group name. A
   * visual abstraction of the data will be created and registered with the
   * visualization. An exception will be thrown if the group name is already in
   * use.
   * 
   ...
   *          the data table to visualize
   * @param filter
   *          a filter Predicate determining which data Tuples in the input table
   *          are visualized
   */
   ...
   * name. An exception will be thrown if the group name is already in use. This
   * method allows you to insert custom implementations of VisualTable into a
   * Visualization. It is intended for advanced users and should <b>NOT</b> be
   * used if you do not know what you are doing. In almost all cases, one of the
   * other add methods is preferred.
   * 
   * @param table
  ...
  /**
   * Adds a graph to this visualization, using the given data group name. A visual
   * abstraction of the data will be created and registered with the
   * visualization. An exception will be thrown if the group name is already in
   * use.
   * 
   * @param group
   *          the data group name for the visualized graph. The nodes and edges
   *          will be available in the "group.nodes" and "group.edges" subgroups.
   * @param graph
   ...
   *          the graph to visualize
   * @param filter
   *          a filter Predicate determining which data Tuples in the input graph
   *          are visualized
   */
  ...
  public synchronized VisualGraph addGraph(String group, Graph graph, Predicate filter, Schema nodeSchema,
    ...
    VisualGraph vg = new VisualGraph(nt, et, graph.isDirected(), graph.getNodeKeyField(), graph.getEdgeSourceField(),
  ...
  /**
   * Adds a tree to this visualization, using the given data group name. A visual
   * abstraction of the data will be created and registered with the
   * visualization. An exception will be thrown if the group name is already in
   * use.
   * 
   * @param group
   *          the data group name for the visualized tree. The nodes and edges
   *          will be available in the "group.nodes" and "group.edges" subgroups.
   * @param tree
   ...
   * VisualTable. This is useful for creating VisualItems that inherit a set of
   * visual properties from another group of VisualItems. This might be used, for
   * example, in the creation of small multiples where only a few visual
   * attributes vary across the multiples.
   * 
   * @param group
   ...
   * @param override
   *          a data schema indicating which data fields should not be inherited,
   *          but managed locally by the derived group
  ...
  /**
   * Add a group of decorators to an existing visual data group. Decorators are
   * VisualItem instances intended to "decorate" another VisualItem, such as
   * providing a label or dedicated interactive control, and are realizeed as
   * {@link prefuse.visual.DecoratorItem} instances that provide access to the
   * decorated item in addition to the standard VisualItem properties. The
   * generated table is created using the
   * {@link #addDerivedTable(String, String, Predicate, Schema)} method, but with
   * no VisualItem properties inherited from the source group.
   * 
   * @param group
   ...
   * providing a label or dedicated interactive control, and are realizeed as
   * {@link prefuse.visual.DecoratorItem} instances that provide access to the
   * decorated item in addition to the standard VisualItem properties.
   * 
   * @param group
   *          the data group to use for the decorators
   * @param source
   *          the source data group to decorate
   * @param schema
   *          schema indicating which variables should <b>not</b> be inherited
   *          from the source data group and instead be managed locally by the
   *          generated VisualTable
   * @return the generated VisualTable of DecoratorItem instances
   */
  ...
  /**
   * Removes a data group from this Visualization. If the group is a focus group,
   * the group will simply be removed, and any subsequent attempts to retrieve the
   * group will return null. If the group is a primary group, it will be removed,
   * and any members of the group will also be removed from any registered focus
   * groups.
   * 
   * @param group
   *          the data group to remove
   * @return true if the group was found and removed, false if the group was not
   ...
   * sets added using the "addXXX" methods will be removed from the visualization.
   * All registered focus groups added using the addFocusGroup() methods will be
   * retained, but will be cleared of all tuples.
   */
  ...
  /**
   * Get the Tuple from a backing source data set that corresponds most closely to
   * the given VisualItem.
   * 
   * @param item
   ...
   * 
   * @param group
   *          the data group from which to lookup the source tuple, only primary
   *          visual groups are valid, focus groups will not work
   * @param t
   *          the source data tuple
   * @return the associated VisualItem from the given data group, or null if no
   *         such VisualItem exists
   */
  ...
  /**
   * Add a new secondary, or focus, group to this visualization. By default the
   * added group is an instance of {@link prefuse.data.tuple.DefaultTupleSet}.
   * 
   * @param group
  ...
  /**
   * Retrieve the visual data group of the given group name. Only primary visual
  ...
  /**
   * Invalidate the bounds of all VisualItems in the given group. This will cause
   ...
   * @param expr
   *          an expression string that should parse to a Predicate indicating
   *          which items should be included in the iteration. The input string
   *          will be parsed using the
  ...
  /**
   * Set a data field value for all items in a given data group matching a given
   * filter predicate.
   * 
   * @param group
   *          the visual data group name
  ...
  /**
   * Sets the interactivity status for all items in a given data group matching a
   * given filter predicate.
   * 
   * @param group
   *          the visual data group name
  ...
  /**
   * Remove a data processing Action registered with this visualization. If the
   * removed action is currently running, it will be canceled. The visualization
   * reference held by the removed Action will be set to null.<br/>
   * <strong>NOTE:</strong> Errors may occur if the removed Action is included in
   * an "always run after" relation with another registered Action that has not
   * been removed from this visualization. It is the currently the responsibility
   * of clients to avoid this situation.
   * 
   * @param name
   *          the name of the Action
  ...
  /**
   * Schedule the Action with the given name to run immediately. The running of
   * all Actions is managed by the {@link prefuse.activity.ActivityManager}, which
  ...
  /**
   * Schedule the Action with the given name to run after the specified delay. The
   * running of all Actions is managed by the
   * {@link prefuse.activity.ActivityManager}, which runs in a dedicated thread.
   * 
   * @param action
   *          the name of the Action to run
   * @param delay
   *          the amount of time to wait, in milliseconds, before running the
   *          Action
   * @return the Action scheduled to run
   */
   ...
   *          the name of the Action to run
   * @param startTime
   *          the absolute system time, in milliseconds since the epoch, at which
   *          to run the Action.
   * @return the Action scheduled to run
   */
  ...
  /**
   * Schedule the Action with the given name to run after another Action finishes
   * running. This relationship will only hold for one round of scheduling. If the
   * "before" Action is run a second time, the "after" action will not be run a
   * second time. The running of all Actions is managed by the
   * {@link prefuse.activity.ActivityManager}, which runs in a dedicated thread.
   * 
   * @param before
  ...
  /**
   * Set the RendererFactory used by this Visualization. The RendererFactory is
   * responsible for providing the Renderer instances used to draw the
   * VisualItems.
   * 
   * @param rf
  ...
  /**
   * Get the display at the given list index. Displays are numbered by the order
   * in which they are added to this visualization.
   * 
   * @param idx
   *          the list index
  ...
  /**
   * Report damage to associated displays, indicating a region that will need to
   * be redrawn.
   * 
   * @param item
