...
public class GraphicsLib {
   ...
   * 
   * The running time of this algorithm is O(n log n), where n is the number of
   * input points.
   * 
   * @param pts
   ...
   * @param amt
   *          the amount by which to expand the polygon, each point will be moved
   *          this distance along the line from the centroid of the polygon to the
   *          given point.
   */
   ...
   * ensuring the connected spline segments form a differentiable curve, ensuring
   * at least a minimum level of smoothness.
   * 
   * @param pts
   *          the points to interpolate with a cardinal spline
   * @param slack
   *          a parameter controlling the "tightness" of the spline to the control
   *          points, 0.10 is a typically suitable value
   * @param closed
   *          true if the cardinal spline should be closed (i.e. return to the
   ...
   *         instance.
   */
  public static GeneralPath cardinalSpline(GeneralPath p, float pts[], float slack, boolean closed, float tx,
      float ty) {
    int npoints = 0;
   ...
   * @param epsilon
   *          threshold value under which to treat the difference between two
   *          values to be zero. Used to determine which segments to treat as
   *          lines rather than curves.
   * @param slack
   *          a parameter controlling the "tightness" of the spline to the control
   *          points, 0.10 is a typically suitable value
   * @param closed
   *          true if the spline should be closed (i.e. return to the starting
   *          point), false for an open curve
   * @param tx
   *          a value by which to translate the curve along the x-dimension
   * @param ty
   *          a value by which to translate the curve along the y-dimension
   ...
   *         instance.
   */
  public static GeneralPath stackSpline(GeneralPath p, float pts[], int start, int npoints, float epsilon, float slack,
      boolean closed, float tx, float ty) {
    // compute the size of the path
    int len = 2 * npoints;
    int end = start + len;
  ...
  /**
   * Sets a VisualItem's bounds based on its shape and stroke type. This method is
   * optimized to avoid calling .getBounds2D where it can, thus avoiding object
   * initialization and reducing object churn.
   * 
   * @param item
   ...
   * @param stroke
   *          the stroke type that will be used for drawing the object, and may
   *          affect the final bounds. A null value indicates the default (line
   *          width = 1) stroke is used.
   */
   ...
   * method uses the {@link java.awt.Graphics} interface methods when it can, as
   * opposed to the {@link java.awt.Graphics2D} methods such as
   * {@link java.awt.Graphics2D#draw(java.awt.Shape)} and
   * {@link java.awt.Graphics2D#fill(java.awt.Shape)}, resulting in a significant
   * performance increase on the Windows platform, particularly for rectangle and
   * line drawing calls.
   * 
   * @param g
   *          the graphics context to render to
   * @param item
   *          the item being represented by the shape, this instance is used to
   *          get the correct color values for the drawing
   * @param shape
   *          the shape to render
   * @param stroke
   *          the stroke type to use for drawing the object.
   * @param type
   *          the rendering type indicating if the shape should be drawn, filled,
   *          or both. One of
   ...
   *          or {@link prefuse.render.AbstractShapeRenderer#RENDER_TYPE_NONE}.
   */
