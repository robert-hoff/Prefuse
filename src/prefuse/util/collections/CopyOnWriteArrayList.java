...
public class CopyOnWriteArrayList implements List, RandomAccess, Cloneable, java.io.Serializable {
  ...
  /**
   * This has been made public to support more efficient iteration. <strong>DO NOT
   * MODIFY this array upon getting it</strong>. Otherwise you risk wreaking havoc
   * on your list. In fact, if you are not the author of this comment, you
   ...
   * provided array, starting at position <tt>first</tt>. To copy an entire array,
   * call with arguments (array, 0, array.length).
   * 
   * @param toCopyIn
   *          the array. A copy of the indicated elements of this array is used as
   *          the internal array.
   * @param first
   *          The index of first position of the array to start copying from.
   * @param n
   *          the number of elements to copy. This will be the new size of the
   *          list.
   */
   ...
   * list, searching forwards from <tt>index</tt>, or returns -1 if the element is
   * not found. More formally, returns the lowest index <tt>i</tt> such that
   ...
   * list, searching backwards from <tt>index</tt>, or returns -1 if the element
   * is not found. More formally, returns the highest index <tt>i</tt> such that
   * <tt>(i&nbsp;&lt;=&nbsp;index&nbsp;&amp;&amp;&nbsp;(e==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;e.equals(get(i))))</tt>,
   * or -1 if there is no such index.
   *
   * @param e
   ...
   *         or equal to <tt>index</tt> in this list; -1 if the element is not
   *         found.
   * @throws IndexOutOfBoundsException
   ...
   * determining the length of this list <i>only</i> if the caller knows that this
   ...
   * <tt>fromIndex</tt>, inclusive, and <tt>toIndex</tt>, exclusive. Shifts any
   ...
   * @throws IndexOutOfBoundsException
   *           if fromIndex or toIndex out of range (fromIndex &lt; 0 || fromIndex
   ...
   * and only if the specified object is also a {@link List}, both lists have the
   * same size, and all corresponding pairs of elements in the two lists are
   * <em>equal</em>. (Two elements <tt>e1</tt> and <tt>e2</tt> are <em>equal</em>
   * if <tt>(e1==null ?
   * e2==null : e1.equals(e2))</tt>.) In other words, two lists are defined to be
   ...
   * iterator. The iterator does <em>NOT</em> support the <tt>remove</tt> method.
   *
   ...
   * The returned iterator provides a snapshot of the state of the list when the
   * iterator was constructed. No synchronization is needed while traversing the
   * iterator. The iterator does <em>NOT</em> support the <tt>remove</tt>,
   * <tt>set</tt> or <tt>add</tt> methods.
   */
  @Override
   ...
   * <tt>UnsupportedOperationException</tt> in its <tt>remove</tt>, <tt>set</tt>
   * and <tt>add</tt> methods.
   *
   * @throws IndexOutOfBoundsException
   *           {@inheritDoc}
   */
  ...
  private static class COWIterator implements ListIterator {
     ...
     *           always; <tt>remove</tt> is not supported by this iterator.
     */
    @Override
    public void remove() {
   ...
   * inclusive, and <tt>toIndex</tt>, exclusive. The returned list is backed by
   * this list, so changes in the returned list are reflected in this list, and
   * vice-versa. While mutative operations are supported, they are probably not
   ...
   * change the size of the list, or otherwise perturb it in such a fashion that
