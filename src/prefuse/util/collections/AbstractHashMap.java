...
public abstract class AbstractHashMap {
   ...
   * <tt>c * minLoadFactor <= s <= c * maxLoadFactor</tt>, where s=size() is the
   * number of associations currently contained. The term "c * minLoadFactor" is
   * called the "lowWaterMark", "c * maxLoadFactor" is called the "highWaterMark".
   * In other words, the table capacity (and proportionally the memory used by
   * this class) oscillates within these constraints. The terms are precomputed
   * and cached to avoid recalculating them each time put(..) or removeKey(...) is
   * called.
   */
  ...
  /**
   * Chooses a new prime table capacity optimized for growing that (approximately)
   * satisfies the invariant
   * <tt>c * minLoadFactor <= size <= c * maxLoadFactor</tt> and has at least one
   * FREE slot for the given size.
   */
