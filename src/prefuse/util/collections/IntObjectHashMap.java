...
public class IntObjectHashMap extends AbstractHashMap implements Cloneable {
  ...
  /**
   * Constructs an empty map with the specified initial capacity and default load
   * factors.
   * 
  ...
  /**
   * Constructs an empty map with the specified initial capacity and the specified
   * minimum and maximum load factor.
   * 
   * @param initialCapacity
   ...
   * associations without needing to allocate new internal memory. If necessary,
   * allocates new internal memory and increases the capacity of the receiver.
   * <p>
   * This method never need be called; it is for performance tuning only. Calling
   * this method before <tt>put()</tt>ing a large number of associations boosts
   * performance, because the receiver will grow only once instead of potentially
  ...
  /**
   * Returns the value associated with the specified key. It is often a good idea
   * to first check with {@link #containsKey(int)} whether the given key has a
   * value associated or not, i.e. whether there exists an association for the
   * given key or not.
   * 
   * @param key
   *          the key to be searched for.
   * @return the value associated with the specified key; <tt>null</tt> if no such
   *         key is present.
   */
   ...
   *         already contained at slot -index-1. If the returned index >= 0, then
   ...
   * @return the index where the key is contained in the receiver, returns -1 if
   ...
   * @return the index where the value is contained in the receiver, returns -1 if
  ...
  /**
   * Returns the first key the given value is associated with. It is often a good
   * idea to first check with {@link #containsValue(Object)} whether there exists
   * an association from a key to this value.
   * 
   * @param value
  ...
  /**
   * Fills all keys contained in the receiver into the specified list. Fills the
   * list, starting at index 0. After this call returns the specified list has a
   * new size that equals <tt>this.size()</tt>.
   * <p>
   ...
   * larger capacity. This method is called automatically when the number of keys
   * in the receiver exceeds the high water mark or falls below the low water
   * mark.
   */
  ...
  protected void setUp(int initialCapacity, double minLoadFactor, double maxLoadFactor) {
    ...
    // establishing it now (upon instance construction) would immediately make the
    // table shrink upon first put(...).
    // After all the idea of an "initialCapacity" implies violating lowWaterMarks
    // when an object is young.
    // See ensureCapacity(...)
    this.lowWaterMark = 0;
  ...
  public void trimToSize() {
    // * 1.2 because open addressing's performance exponentially degrades beyond
    // that point
  ...
  /**
   * Fills all values contained in the receiver into the specified list. Fills the
   * list, starting at index 0. After this call returns the specified list has a
   * new size that equals <tt>this.size()</tt>.
   * <p>
