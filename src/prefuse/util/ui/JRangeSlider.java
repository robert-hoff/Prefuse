...
public class JRangeSlider extends JComponent implements MouseListener, MouseMotionListener, KeyListener {
   ...
   * Bederson, Jesse Grosjean, and Jon Meyer as part of an HCIL Tech Report. It is
   * modified to allow both vertical and horitonal modes. It also fixes a bug with
   * offset on the buttons. Also fixed a bug with rendering using (x,y) instead of
   * (0,0) as origin. Also modified to render arrows as a series of lines rather
   * than as a GeneralPath. Also modified to fix rounding errors on toLocal and
   * toScreen.
   * 
   ...
   * parameterization of rendering/appearance options. Furthermore, a stub method
   * has been introduced to allow subclasses to perform custom rendering within
   * the slider through.
   */
  ...
  /**
   * Sets the low value shown by this range slider. This causes the range slider
   * to be repainted and a ChangeEvent to be fired.
   * 
   * @param lowValue
  ...
  /**
   * Sets the high value shown by this range slider. This causes the range slider
   * to be repainted and a ChangeEvent to be fired.
   * 
   * @param highValue
  ...
  /**
   * Sets the minimum extent (difference between low and high values). This method
   * <strong>does not</strong> change the current state of the model, but can
   * affect all subsequent interaction.
   * 
   * @param minExtent
  ...
  /**
   * Get the slider thumb color. This is the part of the slider between the range
   * resize buttons.
   * 
  ...
  /**
   * Set the slider thumb color. This is the part of the slider between the range
   * resize buttons.
   * 
   * @param thumbColor
  ...
  public void paintComponent(Graphics g) {
    ...
    if (orientation == VERTICAL) {
      if (direction == LEFTRIGHT_TOPBOTTOM) {
        ...
        paintArrow(g2, (width - ARROW_WIDTH) / 2.0, min - ARROW_SZ + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH,
            ARROW_HEIGHT, true);
        paintArrow(g2, (width - ARROW_WIDTH) / 2.0, max + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH, ARROW_HEIGHT,
            false);
      } else {
        g2.setColor(getForeground());
        ...
        // Draw arrows
        g2.setColor(Color.black);
        paintArrow(g2, (width - ARROW_WIDTH) / 2.0, min + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH, ARROW_HEIGHT,
            false);
        paintArrow(g2, (width - ARROW_WIDTH) / 2.0, max - ARROW_SZ + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH,
            ARROW_HEIGHT, true);
      }
    } else {
      if (direction == LEFTRIGHT_TOPBOTTOM) {
        ...
        // Draw arrows
        g2.setColor(Color.black);
        paintArrow(g2, min - ARROW_SZ + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT,
            ARROW_WIDTH, true);
        paintArrow(g2, max + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT, ARROW_WIDTH,
            false);
      } else {
        g2.setColor(getForeground());
        ...
        // Draw arrows
        g2.setColor(Color.black);
        paintArrow(g2, min + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT, ARROW_WIDTH,
            true);
        paintArrow(g2, max - ARROW_SZ + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT,
            ARROW_WIDTH, false);
      }
    }
  }
  ...
  /**
   * This draws an arrow as a series of lines within the specified box. The last
   * boolean specifies whether the point should be at the right/bottom or
   * left/top.
   */
  ...
  protected int toScreen(int xOrY) {
    ...
    // Otherwise, we have to invert the number by subtracting the value from the
    // height
    if (direction == LEFTRIGHT_TOPBOTTOM) {
