...
public class JSearchPanel extends JPanel implements DocumentListener, ActionListener {
   ...
   *          indicates if each keystroke event should result in a new search
   *          being issued (true) or if searches should only be initiated by
   *          hitting the enter key (false)
   */
  ...
  /**
   * Create a new JSearchPanel. The default search tuple set for the visualization
   * will be used.
   * 
   * @param vis
   ...
   *          the data field being searched
   * @param autoIndex
   *          indicates if items should be automatically indexed and unindexed as
   *          their membership in the source group changes.
   */
  ...
  /**
   * Set the fill color of the cancel 'x' button that appears when the button has
  ...
  /**
   * A button depicted as an "X" that allows users to cancel the current query and
   * clear the query field.
   */
  public class CancelButton extends JComponent implements MouseListener {

    private boolean hover = false;
    private int[] outline = new int[] { 0, 0, 2, 0, 4, 2, 5, 2, 7, 0, 9, 0, 9, 2, 7, 4, 7, 5, 9, 7, 9, 9, 7, 9, 5, 7, 4,
        7, 2, 9, 0, 9, 0, 7, 2, 5, 2, 4, 0, 2, 0, 0 };
