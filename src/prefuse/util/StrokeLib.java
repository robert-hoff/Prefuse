...
public class StrokeLib {
   ...
   *          the requested stroke width
   * @param dashes
   *          an array describing the alternation pattern of a dashed line. For
   *          example [5f, 3f] will create dashes of length 5 with spaces of
   *          length 3 between them. A null value indicates no dashing.
   * @return the stroke
  ...
  /**
   * Get a dashed stroke of the given width, cap, join, miter limit, and dashing
   * attributes.
   * 
   * @param width
   *          the requested stroke width
  ...
  public static BasicStroke getDerivedStroke(BasicStroke s, float width) {
    if (s.getLineWidth() == width) {
      ...
      return getStroke(width * s.getLineWidth(), s.getEndCap(), s.getLineJoin(), s.getMiterLimit(), s.getDashArray(),
          s.getDashPhase());
    }
  }
